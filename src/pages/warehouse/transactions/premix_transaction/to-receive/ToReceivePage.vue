<template>
  ...
  <div v-else>
    <div
      v-if="toReceivePremixData.length === 0"
    ...
    <q-scroll-area v-else style="height: 450px; max-width: 1500px">
      <div class="q-gutter-md q-ma-md">
        <q-card
          v-for="(toReceive, index) in toReceivePremixData"
          :key="index"
          @click="handleDialog(toReceive)"
            ...
            <div class="row items-start justify-between">
              <div class="col-8 column q-gutter-y-xs">
                <div class="text-body1 text-weight-bold text-primary-dark">
                  Premix: {{ capitalize(toReceive.name) || "-" }}
                </div>
                <div class="text-caption text-grey-6 text-weight-medium">
                  {{ formatTimestamp(toReceive.created_at) }}
                </div>
              </div>
              ...
              <div class="col-4 column items-end q-gutter-y-xs">
                <div>
                  <q-badge
                    class="text-weight-bold q-px-md q-py-sm to-receive-badge text-uppercase"
                  >
                    {{ toReceive.status }}
                  </q-badge>
                </div>
                <div class="text-body2 text-weight-bold text-grey-8 q-pt-sm">
                  {{ toReceive.branch_premix.branch_recipe.branch.name || "-" }}
                </div>
              </div>
            </div>
            ...
            <div class="row items-end justify-between">
              <div class="column q-gutter-y-xs">
                ...
                <div class="text-body2 text-weight-semibold text-grey-9">
                  {{ formatFullname(toReceive.employee) || "-" }}
                </div>
              </div>
              <div class="col-5 column">
                <div class="text-caption text-grey-7">To Received By:</div>
                <div class="text-body2 text-weight-semibold text-grey-9">
                  {{ formatFullname(toReceive.history[0].employee) || "-" }}
                </div>
              </div>
            </div>
          </q-card-section>
...
const toReceivePremixData = computed(() => premixStore.toReceivePremixData);
console.log("toReceivePremixData", toReceivePremixData.value);
...
onMounted(async () => {
  if (warehouseId) {
    await fetchToReceivePremix(warehouseId);
  }
});
...
const fetchToReceivePremix = async () => {
  try {
    loading.value = true;
    await premixStore.fetchToReceivePremix(warehouseId, status.value);
    if (!toReceivePremixData.value.length) {
...
.text-caption {
  font-size: 0.7rem; /* ⬇️ Reduced from 0.9rem */
  color: $text-muted;
}
