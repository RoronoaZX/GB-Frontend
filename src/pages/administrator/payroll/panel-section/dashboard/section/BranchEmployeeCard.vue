      ...
      <div class="row items-center justify-between">
        <div>
          <div class="text-h6 text-weight-bold text-dark row items-center">
            <q-icon name="storefront" class="q-mr-sm text-teal-8" size="24px" />
            Branch Employees
          </div>
          <div class="text-caption text-grey-6 q-mt-xs">
            Total Locations: <span class="text-weight-bold text-teal-8">{{ branches.length }}</span>
          </div>
        </div>
      </div>
    </q-card-section>
      ...
      <div class="vertical-scroll-container">
        ...
        >
          <div class="row items-center col-grow">
            <div class="list-icon-wrapper bg-teal-1 text-teal-8 q-mr-md">
          ...
          <div class="row items-center q-pr-sm">
            <q-badge rounded color="teal-1" text-color="teal-9" class="text-weight-bold q-px-md q-py-xs">
              {{ branch?.branch_employee?.length || 0 }} staff
            </q-badge>
          </div>
        </div>
      </div>
...
const goToBranch = (branchId) => {
  router.push({ name: "BranchDetail", params: { branch_id: branchId }, query: { from: "payroll" } });
};
</script>
