      ...
      <q-card-section class="q-pt-sm q-pb-md">
        <q-card flat bordered class="summary-card q-pa-md">
          <q-list dense>
            ...
            <q-item>
              ...
              <q-item-section>
                <q-item-label class="text-body1 text-weight-meduim">
                  ...
                  <span class="text-accent">{{ regularPay }}</span>
                </q-item-label>
              </q-item-section>
            </q-item>
            ...
            <q-item>
              <q-item-section avatar>
                <q-icon name="attach_money" color="cyan-9" />
              </q-item-section>
              <q-item-section>
                <q-item-label class="text-body1 text-weight-bold">
                  Overall Total Overtime Cost :
                  <span class="text-cyan-9">
                    {{ overallOvertimeCost }}
                  </span>
                </q-item-label>
              </q-item-section>
            ...
            <q-item>
              ...
              <q-item-section>
                <q-item-label class="text-body1 text-weight-bold">
                  Overall Total Undertime / Late Cost :
        ...
        <!-- <div class="row justify-between">
          <div class="text-bold text-primary">
            ...
            <div>Total Number of Days: {{ dtrRecord.records.length }}</div>
            <div>Expected Salary : {{ expectedSalary }}</div>
            <div>
              Overall Total Working Hours: {{ overallTotalWorkingHours }}
            </div>
            <div>
              Overall Total Undertime / Late: {{ overallTotalUndertime }}
            </div>
            <div>Overall Total Salary: {{ overallTotalSalary }}</div>
            <div>Overall Total Undertime Cost: {{ overallUndertimeCost }}</div>
          </div>
          <div>
            <div class="text-bold text-primary">
              Schedule :
              {{
                `${employeesData?.designation?.time_in || "--:--"} - ${
                  employeesData?.designation?.time_out || "--:--"
                }`
              }}
            </div>
          </div>
...
//       console.log("=================== row.ot_status", row.ot_status);
//       console.log("=================== row.overtime_in", row.overtime_in);
//       console.log("=================== row.overtime_out", row.overtime_out);
//       // <-- The problem is here
...
onMounted(async () => await fetchEmployeeDetails());
...
// Helper function to parse time strings (e.g., "08:00 AM") and set them on a specific date.
// This is crucial for accurate time comparisons across days.
const parseTimeToDate = (timeString, dateObj) => {
  if (!timeString || !dateObj) return null;

  // A simple regex to extract hours and minutes and AM/PM part
  const match = timeString.match(/(\d+):(\d+)\s*(AM|PM)?/i);
  if (!match) return null;
  ...
  // Adjust hours for PM times, ensuring 12 PM is 12 and 12 AM is 0
  if (ampm === "PM" && hours < 12) {
    hours += 12;
  } else if (ampm === "AM" && hours === 12) {
  ...
  const newDate = new Date(dateObj); // Create a new date object to avoid modifying the original
  newDate.setHours(hours, minutes, 0, 0); // Set hours, minutes, seconds, milliseconds
  return newDate;
};

// Function to convert total minutes into "Xh, Ym" format
const formatMinutesToHoursMinutes = (totalMinutes) => {
  if (totalMinutes < 0) return "0h 0m"; //  Ensure positive for display
  const hours = Math.floor(totalMinutes / 60);
  const minutes = totalMinutes % 60;
  return `${hours}h ${minutes}m`;
};

// Function to convert total minutes into "Xh Ym" string to total minutes
const convertHoursMinutesToMinutes = (hmString) => {
  const match = hmString.match(/(\d+)h\s*(\d+)m/);
  if (match) {
    const hours = parseInt(match[1], 10);
...
const overallTotalUndertime = computed(() => {
  ...
  dtrRows.forEach((row) => {
    ...
    // Calculate expected working mi nutes based on scheduled times, minus 1 hour lunch
    const expectedMs = scheduledOut.getTime() - scheduledIn.getTime();
    const expectedWorkingMinutes = Math.floor(
      (expectedMs - 1000 * 60 * 60) / (1000 * 60)
    );
...
const overallTotalOvertime = computed(() => {
  ...
  dtrRows.forEach((row) => {
    console.log("overallTotalOvertime", row);
    // Only consider approved overtime
    if (row.ot_status === "approved" && row.overtime_in && row.overtime_out) {
      const otIn = new Date(row.overtime_in);
      const otOut = new Date(row.overtime_out);
...
// // Computed property overall total overtime
// const overallTotalOvertime = computed(() => {
//   let totalOvertimeMInutes = 0,
...
// NEW Computed Property: Overall Total Salary
const overallTotalSalary = computed(() => {
  if (
    !employeesData.value ||
    !employeesData.value.employment_type ||
  ...
  // Get the total working hours in minutes from the prevously calculated computed property
  const totalWorkingMinutes = convertHoursMinutesToMinutes(
    overallTotalWorkingHours.value
  );
  // Get the total overtime hours in minutes
  ...
  // Assuming overtime is paid at the regular hourly rate for simplicity, adjust if rate applies
  const calculatedSalary =
    totalRegularWorkingHours * hourlyRate + totalOvertimeHours * hourlyRate;

  // Convert total working minutes to total working hours
...
const expectedSalary = computed(() => {
  ...
  console.log("props?.dtrRecord?.records :", props?.dtrRecord?.records?.length);
  const ratePerDay = employeesData?.value?.employment_type?.salary || "0";
  const totalNumberOfDays = props?.dtrRecord?.records?.length || "0";
