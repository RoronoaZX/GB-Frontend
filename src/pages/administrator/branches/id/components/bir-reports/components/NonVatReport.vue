<template>
  ...
  <div v-if="loading" class="q-pa-sm">
    <q-markup-table flat class="table-container">
      <thead>
        <tr>
          <th v-for="col in tableColumns" :key="col.name" class="text-center">
      ...
      <tbody>
        <tr v-for="n in 5" :key="n">
          <td v-for="col in tableColumns" :key="col.name" class="text-center">
...
const fetchBranchData = async (branchId) => {
  try {
    ...
    // showNoDataMessage.value = branchData.value.length === 0;
    /* console.log("branchData", branchData.value); */
  } catch (error) {
